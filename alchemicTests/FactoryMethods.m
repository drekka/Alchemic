...
ACRegister(ACIsFactory, ACAsName(@"buildAString"), ACReturnType(NSString), ACFactorySelector(makeAString));
-(NSString *) makeAString {
    x++;
    return [NSString stringWithFormat:@"Factory string %i", x];
}

ACRegister(ACIsFactory, ACAsName(@"buildAComponentString"), ACReturnType(NSString), ACFactorySelector(makeAStringWithComponent:), ACWithClass(Component))
